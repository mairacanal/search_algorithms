...
/*
 * @brief Funcao de busca sequencial que realoca um elementos para a primeira posicao durante as buscas.
 * @return Retorna -1 para valores não encontrados. Retorna a posicao de valores encontrados.
 */
int mover_para_frente(Vetor *tabela, int buscado)
{
  ...
  for (int posicao = 0; posicao < tabela->tamanho; posicao++) {
        ...
        if (tabela->vetor[posicao] == buscado) {
            ...
            // coloca o primeiro valor no inicio
            aux = tabela->vetor[posicao];
            tabela->vetor[posicao] = tabela->vetor[0];
            tabela->vetor[0] = aux;
...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (mover_para_frente(&tabela, consultas[i]) != -1)
          encontrados++;
